---
layout: page
title: Personal Pursuits
description: Outputs from hobbies, passion projects, and creative interests
img: assets/img/projects/xoblob.jpeg
importance: 6
category: archive
related_publications: false
---

## Princédex

A collectible archive of Prince.
Open a pack ever hours, build your collection, and uncover memories from hundreds of photos and edits spanning years of his life.

Start collecting now at: [jackjburnett.github.io/princedex/](https://jackjburnett.github.io/princedex/)

---

## Train the Finish

A workout generator that maps wrestling finishers to targeted strength exercises. Pick a move, filter by available equipment, and generate a personalised workout designed around the muscles that drive the technique. Export routines to Hevy (requires Pro) or save workouts as reference images.

Train now at: [jackjburnett.github.io/train-the-finish/](https://jackjburnett.github.io/train-the-finish)

---

## Main Event

A 2D fighting game in development using [Godot](https://godotengine.org/). Playable prototypes will be released when ready.

---

## Weird Qualifications and Licences

[BARBICIDE Certified](https://certifications.barbicide.com/verify/7A572CFAA5-7A572C5E67-7A51B06594/)

Certified Bear (FT340158) and Mountain Lion (FT340160) Identifier by [Montana Fish, Wildlife & Parks](https://fwp.mt.gov/)

[Certified Rat Tickler]({{ site.url }}/assets/img/projects/personal_projects/rat_tickling.png){:target="\_blank"} from [Purdue University](https://nc3rs.org.uk/3rs-resources/rat-tickling/rat-tickling-certification)

[Ordained Dudeist Priest]({{ site.url }}/assets/img/projects/personal_projects/dudeism.png){:target="\_blank"} at [Dudeism](https://dudeism.com/), the Church of the Latter-Day Dude and [Resident of Dudeland, Macronesia](https://macronesia.com/residency-certificate/?yourname=Jack%20Burnett&resdate=05/08/2026&resstate=Dudeland)

[UK Drone and Model Aircraft Flyer Licence]({{ site.url }}/assets/img/projects/personal_projects/flyer_id.png){:target="\_blank"}

<a href="https://www.dudeism.com" target="_blank"><img alt="I am an Ordained Dudeist Priest" src="https://www.dudeism.com/Images/Im-an-ordained-dudeist-priest.gif" border="0" title="I am an Ordained Dudeist Priest" /></a>
